...
class FlushMan:
    ...
    def __init__(self, wakeup_interval, ds):
        ...
        self.cexe = "/gpfs/alpine/proj-shared/csc143/kmehta/gtc/cheetah/bb-to-pfs-serial-positional"
    ...
    def get_data_files(self, adios_wpn):
        ...
        _data_files = glob.glob(self.nvme_root+"/data*")
        while(len(_data_files) < adios_wpn):  # specfem has not written data yet
            time.sleep(1)
            _data_files = glob.glob(self.nvme_root+"/DATA_RESTART.bp/data*")

        # Sort the data files as the output of glob.glob is arbitrary
        adios_dir = glob.glob(self.nvme_root+"DATA_RESTART.bp")[0]
        ...
        destdir = "./restart_dir1/DATA_RESTART.bp/"
        ...
        # Some ranks may get an additional file
        f_rem = len(data_files)%self.local_commsize
        if f_rem>0:
            if self.local_rank < f_rem:
                df = data_files[self.local_commsize+self.local_rank]
                self.data_files.append(DataFile(df, destdir+os.path.basename(df)))
    ...
    def _end_of_run(self):
        ...
        return os.path.isfile("./FileExit.out")
    ...
    def _copy_adios_md_file(self):
        ...
        md_idx = adios_dir+"/md.0"
        md_idx_dest = os.path.dirname(self.data_files[0].dest) + "/md.0"
